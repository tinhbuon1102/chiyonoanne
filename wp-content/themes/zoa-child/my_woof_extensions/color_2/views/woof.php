...
$color_2 = WOOF_EXT::$includes['taxonomy_type_objects']['color'];
//********************
...
//excluding hidden terms
    ...
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : $inique_id = uniqid(); ?>
            ...
            $color = $color_2->get_term_color($term['term_id'], $term['taxonomy']);
            $color_img = $color_2->get_term_image($term['term_id'], $term['taxonomy']);
            if ($color_img == 'none')
            {
                $color_img = '';
            }
