...
?>
	...
	if (!isset($_REQUEST['image_serie_index']) || !$_REQUEST['image_serie_index'])
	{
		...
		<div class="flex-justify-between row test">
			...
			<div class="col-lg-6 col-sm-12 col-12 portfolio_sideinfo">
				...
				if($galllery!=''){
				?>
					<div class="button_row"><a class="btn btn-primary" href="<?php echo get_permalink(2659);?>?pid=<?php echo $post->ID; ?>"><?php _e('Order Now', 'zoa')?></a></div>
					<?php
				}
				else {
	...
	?>
			...
			<div class="body_row">
				<?php //get description from product
			foreach ($images_series as $loop_series_index => $images_serie){
				if ($serie_index_request == $loop_series_index + 1)
				{
			...
			foreach ($images_series as $loop_series_index => $images_serie){
				if ($serie_index_request == $loop_series_index + 1)
				{
					...
					?>
						<div class="button_row"><a class="btn btn-primary" href="<?php echo get_permalink(2659);?>?pid=<?php echo $post->ID; ?>&serie_index=<?php echo $serie_index_request?>"><?php _e('Order Now', 'zoa')?></a></div>
					<?php
