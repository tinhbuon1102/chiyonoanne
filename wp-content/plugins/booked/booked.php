...
define('BOOKED_WC_VERSION', '1.5'); // Updated in 2.2
...
require_once('includes/appointment_list.php');
...
// END public static function activate
        ...
        public function add_menu() {
            ...
            add_submenu_page('booked-appointments', esc_html__('Appointments List', 'booked'), esc_html__('Appointments List', 'booked'), 'edit_booked_appointments', 'appointments-list', array(&$this, 'admin_appointments_list'));
        ...
        // Booked Pending Appointments List
        public function admin_appointments_list() {
            include(sprintf("%s/templates/appointment-list.php", BOOKED_PLUGIN_DIR));
        }
        ...
        public function booked_add_pending_appt_bubble() {
            ...
            foreach ($submenu as $key => $value) :
                if ($key == 'booked-appointments') :
                    if ($pending) {
                        $submenu[$key][1][0] .= " <span style='position:relative; top:1px; margin:0 0 0 2px' class='update-plugins count-$pending' title='$pending'><span style='padding:0 3px 0 1px; display:inline-block; min-width:5px; text-align:center;' class='update-count'>" . $pending . "</span></span>";
                    }
                    return;
                endif;
            endforeach;
...
if (class_exists('booked_plugin')) {
    ...
    $booking_agent = add_role(
            'booked_booking_agent', esc_html__('Booking Agent', 'booked'), array(
        'read' => true,
            )
    );
