...
//get session from step 2 
$your_info = $_SESSION['your_info'];
$arr_your_info = json_decode($your_info, true);
//
        ...
        if ($name_requirements == 'require_surname' && !$surname):
            ...
            if ($email && is_email($email) && $name || !$email && !$email_required && $name):
                ...
                if (apply_filters('booked_update_appointment_calendar', true)) {
                    if (!empty($calendar_id)): $calendar_term = get_term_by('id', $calendar_id[0], 'booked_custom_calendars');
...
', $errors);
    ...
    elseif ($customer_type == 'current'):
        ...
        //update user info
        update_user_meta($user_id, 'user_firstname', $arr_your_info['user_firstname']);
        update_user_meta($user_id, 'user_lastname', $arr_your_info['user_lastname']);
        update_user_meta($user_id, 'billing_last_name_kana', $arr_your_info['billing_last_name_kana']);
        update_user_meta($user_id, 'billing_first_name_kana', $arr_your_info['billing_first_name_kana']);
        update_user_meta($user_id, 'billing_phone', $arr_your_info['phone']);
    ...
    elseif ($customer_type == 'new'):
        if ($arr_your_info['is_register'] == '1') {//DO YOU WANT TO REGISTER AS CLIENT?
            $name = $arr_your_info['user_firstname']; //esc_attr($_POST['booked_appt_name']);
...
//            else:
            ...
            if (empty($errors)):
                ...
                if (apply_filters('booked_update_appointment_calendar', true)) {
                    if (!empty($calendar_id)): wp_set_object_terms($post_id, $calendar_id, 'booked_custom_calendars');
                    endif;
                }
...
', $errors);
            ...
            $email_required = true; //get_option('booked_require_guest_email_address', false);
//            if ($name_requirements == 'require_surname' && !$surname):
//
//                echo 'error###' . esc_html__('Your full name is required to book an appointment.', 'booked');
//
...
', $errors);
    ...
    $error_message = apply_filters(
            'booked_availability_error_message', esc_html__('Sorry, someone just booked this appointment before you could. Please choose a different booking time.', 'booked')
    );
