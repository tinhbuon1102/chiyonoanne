...
function booked_render_time_select($select_name, $interval, $placeholder, $single = false) {
    ...
    if ($single): $html .= '<option value="allday">' . esc_html__('All Day', 'booked') . '</option>';
    endif;
    do {
...
function booked_render_timeslots($calendar_id = false) {
    ...
    if ($first_day_of_week == 1):
        $sunday_item = array_shift($day_loop);
        $day_loop[] = $sunday_item;
        $sunday_item_array = array_shift($day_loop_english_array);
        $day_loop_english_array[] = $sunday_item_array;
    endif;
                ...
                <td>
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <?php
                                echo '<td class="dayTimeslots" data-day="' . $day_loop_english_array[$key] . '">';
...
function booked_admin_calendar($year = false, $month = false, $calendar_id = false, $size = false) {
    ...
    ?><table class="booked-calendar<?php echo ($size ? ' ' . $size : ''); ?>"<?php echo ($calendar_id ? ' data-calendar-id="' . $calendar_id . '"' : ''); ?> data-monthShown="<?php echo $monthShown; ?>">
        <thead>
            ...
            <tr class="days">
                <?php
                if ($first_day_of_week == 7 && !$hide_weekends): echo '<th>' . date_i18n('D', strtotime('Sunday')) . '</th>';
                endif;
                ?>
                ...
                <?php
                if ($first_day_of_week == 1 && !$hide_weekends): echo '<th>' . date_i18n('D', strtotime('Sunday')) . '</th>';
                endif;
                ?>
            </tr>
        </thead>
        <tbody><?php
            ...
            if ($first_day_of_week == 7): $start = $start + 1;
            endif;
            if ($start > 7): $start = 1;
            endif;
                ...
                for ($x = 1; $x <= 7; $x++) {
                    ...
                    if (($counter - $start) < 0) {
                        ...
                        if (isset($appointments_array[$date]) && !empty($appointments_array[$date])):
                            ...
                            elseif ($appointments_count >= $total_full_count): $classes[] = 'booked';
                            endif;
                        endif;
                    }
...
function booked_admin_calendar_date_content($date, $calendar_id = false) {
    ...
    if ($tabbed):
        ...
        <?php
        // Show the Default calendar to admins, not booking agents
        $is_first_tab = true;
                    ...
                    if (!isset($appointments_array[$appt_id]['guest_name'])):
                        ...
                        if (isset($user_info->ID)):
                            if ($user_info->user_firstname):
                                $user_display = '<a href="#" class="user" data-user-id="' . $appointments_array[$appt_id]['user'] . '"><i class="booked-icon booked-icon-pencil"></i>&nbsp;' . ($user_info->user_lastname ? $user_info->user_lastname . ' ' : '') . $user_info->user_firstname . '</a>';
                        ...
                        $user_display = '<a href="#" class="user" data-user-id="0"><i class="booked-icon booked-icon-pencil"></i>&nbsp;' . $appointments_array[$appt_id]['guest_surname'] . ' ' . $appointments_array[$appt_id]['guest_name'] . '</a>';
                    endif;
        ...
        if (!empty($appointments_array)):
            ...
            foreach ($appointments_array as $appointment):
                ...
                if (!isset($appointment['guest_name'])):
                    ...
                    if (isset($user_info->ID)):
                        if ($user_info->user_firstname):
                            $user_display = '<a href="#" class="user" data-user-id="' . $appointment['user'] . '">' . ($user_info->user_lastname ? $user_info->user_lastname . ' ' : '') . $user_info->user_firstname . '</a>';
                        elseif ($user_info->display_name):
                    ...
                    $user_display = '<a href="#" class="user" data-user-id="0">' . $appointment['guest_surname'] . ' ' . $appointment['guest_name'] . '</a>';
                endif;
...
function booked_render_custom_fields($calendar = false) {
            ...
            if (!empty($custom_fields)):
                ...
                foreach ($custom_fields as $field):
                    ...
                    $reset_subs = apply_filters(
                            'booked_custom_fields_add_template_subs', $field_type, $field['name'], $field['value'], $look_for_subs
                    );
            ...
            switch ($field_type):
                ...
                case 'single-line-text-label' :
                    ?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
                        ...
                        <p><input class="cf-required-checkbox"<?php
                            if ($is_required): echo ' checked="checked"';
                            endif;
                            ?> type="checkbox" name="required---<?php echo $numbers_only; ?>" id="required---<?php echo $numbers_only; ?>"> <label for="required---<?php echo $numbers_only; ?>"><?php esc_html_e('Required Field', 'booked'); ?></label></p>
                                    ...
                                    ?><li class="ui-state-default"><i class="main-handle booked-icon booked-icon-bars"></i>
                                        ...
                                        default:
                                            $look_for_subs_action = apply_filters(
                                                    'booked_custom_fields_add_template_main', false, // default value to return when there is no addon plugin to hook on it
                                                    $field_type, $field['name'], $field['value'], $is_required, $look_for_subs, $numbers_only
                                            );
    ...
    <?php
    do_action('booked_custom_fields_add_buttons');
}
