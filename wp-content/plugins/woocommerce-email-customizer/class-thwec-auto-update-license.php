...
if ( ! class_exists( 'THWEC_Auto_Update_License' ) ) {
	class THWEC_Auto_Update_License {
		...
		public $thpl_license_setting_page;
		...
		public function __construct( $file, $software_title, $software_version, $plugin_or_theme, $api_url, $text_domain, $extra ) {
			...
			if ( is_admin() ) {
				...
				$this->thpl_license_setting_page = admin_url('admin.php?page=th_email_customizer_pro&tab=license_settings');
		...
		public function activation() {
			if ( get_option( $this->ame_data_key ) === false || get_option( $this->ame_instance_key ) === false || get_option( $this->ame_product_id_key ) === false ) {
		...
		public function inactive_notice() { ?>
            ...
            <div class="notice notice-error">
                <p><?php printf( __( 'The <strong>%s</strong> License has not been activated. %sClick here%s to activate <strong>%s</strong>.', $this->text_domain ), 
				esc_attr( $this->software_title ), '<a href="'. esc_url($this->thpl_license_setting_page) .'">', '</a>', esc_attr( $this->software_title ) ); ?></p>
            </div>
			<?php
		}
		...
		public function load_settings() {
			...
			// API Key
			add_settings_section( $this->ame_api_key, __( 'License Activation', $this->text_domain ), array(
				$this,
				'wc_am_api_key_text'
			), $this->ame_activation_tab_key );
			add_settings_field( 'status', __( 'License Status', $this->text_domain ), array(
				$this,
				'wc_am_api_key_status'
			), $this->ame_activation_tab_key, $this->ame_api_key );
			add_settings_field( $this->ame_api_key, __( 'License Key', $this->text_domain ), array(
				$this,
				'wc_am_api_key_field'
			), $this->ame_activation_tab_key, $this->ame_api_key );
			add_settings_field( $this->ame_activation_email, __( 'License Email', $this->text_domain ), array(
				$this,
			...
			add_settings_section( 'deactivate_button', __( 'License Deactivation', $this->text_domain ), array(
				$this,
				'wc_am_deactivate_text'
			), $this->ame_deactivation_tab_key );
			add_settings_field( 'deactivate_button', __( 'Deactivate License Key', $this->text_domain ), array(
				$this,
		...
		public function wc_am_api_key_status() {
			...
			$license_status_check = ( ! empty( $license_status[ 'status_check' ] ) && $license_status[ 'status_check' ] == 'active' ) ? '<span class="status-active">Activated</span>' : '<span class="status-deactive">Deactivated</span>';
			...
			if ( $options == 'on' && $activation_status == 'Activated' && $this->ame_options[ $this->ame_api_key ] != '' && $this->ame_options[ $this->ame_activation_email ] != '' ) {
				...
				if ( $activate_results[ 'deactivated' ] === true ) {
					...
					if ( ! empty( $this->ame_activated_key ) ) {
						...
						add_settings_error( 'wc_am_deactivate_text', 'deactivate_msg', __( 'License Key deactivated. ', $this->text_domain ) . "{$activate_results['activations_remaining']}.", 'updated' );
					}
		...
		public function wc_am_deactivate_textarea() {
			...
			echo '/>';
			?><span class="description"><?php _e( 'Deactivates an License Key so it can be used on another blog.', $this->text_domain ); ?></span>
			<?php
		}
		...
		public function create_software_api_url( $args ) {
			$query_string = apply_filters('thwec_api_license_query_string', http_build_query( $args, null, '&' ), $args);
			return add_query_arg( 'wc-api', 'am-software-api', $this->api_url ) . '&' . $query_string;
		}
		...
		private function create_upgrade_api_url( $args ) {
			$query_string = apply_filters('thwec_api_license_query_string', http_build_query( $args, null, '&' ), $args);
			return add_query_arg( 'wc-api', 'upgrade-api', $this->api_url ) . '&' . $query_string;
		}
