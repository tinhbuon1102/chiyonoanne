 ...
 * @since	1.3
 **/
class ParadoxTitanFrameworkOptionCode extends ParadoxTitanFrameworkOption {
	...
	public function printJS() {
		...
		if ( ParadoxTitanFrameworkOption::TYPE_META == $this->type ) {
			return;
		}
	 ...
	 * ParadoxTitanFrameworkCSS, using tf_generate_css_code
	 *
	 * @param	string               $css The CSS to output
	 * @param	ParadoxTitanFrameworkOption $option The option object being generated
	 * @return	void
	 * @since	1.3
	 */
	public function generateCSSCode( $css, $option ) {
		...
		if ( ParadoxTitanFrameworkOption::TYPE_META != $option->type ) {
			$css = $this->getValue();
		}
		return $css;
	}
	...
	public function loadAdminScripts() {
		...
		wp_enqueue_script(
			...
			ParadoxTitanFramework::getURL( '../js/ace-min-noconflict/theme-' . $this->settings['theme'] . '.js',
			__FILE__ )
		);
		wp_enqueue_script(
			'tf-ace-mode-' . $this->settings['lang'],
			ParadoxTitanFramework::getURL( '../js/ace-min-noconflict/mode-' . $this->settings['lang'] . '.js',
			__FILE__ )
		);
	}
	...
	public function registerCustomizerControl( $wp_customize, $section, $priority = 1 ) {
		$wp_customize->add_control( new ParadoxTitanFrameworkOptionCodeControl( $wp_customize, $this->getID(), array(
 ...
 */
add_action( 'customize_register', 'registerParadoxTitanFrameworkOptionCodeControl', 1 );
 ...
 * @since	1.3
 */
function registerParadoxTitanFrameworkOptionCodeControl() {
	class ParadoxTitanFrameworkOptionCodeControl extends WP_Customize_Control {
		public $description;
		public $lang;
		public $theme;
		public $height;
