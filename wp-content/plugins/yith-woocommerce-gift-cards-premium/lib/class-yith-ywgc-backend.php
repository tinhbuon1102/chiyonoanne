...
if ( ! class_exists ( 'YITH_YWGC_Backend' ) ) {
	...
	class YITH_YWGC_Backend {
        ...
        public function ywgc_custom_condition_to_create_gift_card_call_back( $cond, $order ) {
            ...
            if ( $order->get_status() == 'processing' && $gateway->method_title == 'Cash on delivery' )
                return false;
		...
		public function create_gift_cards_for_order( $order ) {
			...
			foreach ( $order->get_items ( 'line_item' ) as $order_item_id => $order_item_data ) {
				...
				for ( $i = 0; $i < $quantity; $i ++ ) {
					...
					if ( (! $gift_card->is_digital && YITH_YWGC ()->enable_pre_printed) || apply_filters('yith_wcgc_force_generate_manually_code_always',false) ) {
					...
					if ( ! $is_postdated && apply_filters( 'ywgc_send_gift_card_code_by_default', true ) ) {
