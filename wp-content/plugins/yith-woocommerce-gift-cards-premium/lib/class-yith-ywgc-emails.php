...
if ( ! class_exists ( 'YITH_YWGC_Emails' ) ) {
	...
	class YITH_YWGC_Emails {
		...
		public function send_gift_card_email( $gift_card, $only_new = true ) {
			...
			if ( (! $gift_card->is_virtual () || empty( $gift_card->recipient )) && apply_filters('yith_wcgc_deny_gift_card_email',true, $gift_card ) ) {
		...
		public function show_link_for_cart_discount( $gift_card ) {
            ...
            if ( "no" != get_option ( "ywgc_auto_discount_button_activation" ) ){
                ...
                if ( get_option ( 'ywgc_auto_discount' ) != 'no' )
                    $args = array(
                        YWGC_ACTION_ADD_DISCOUNT_TO_CART => $gift_card->gift_card_number,
                        YWGC_ACTION_VERIFY_CODE          => YITH_YWGC ()->hash_gift_card ( $gift_card ),
                    );

                if ( get_option ( 'ywgc_email_button_redirect' ) == 'customize_page' )
                    $apply_discount_url = esc_url ( add_query_arg ( $args, get_page_link( get_option ( 'ywgc_redirected_page' ) ) ) );
                else
