...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            area = {
                ...
                height: 0,
                width: 0,
                type_area: '' // textArea, imageArea, barCode
            },
            textArea = {
                content: 'origin',
                size: 14,
            ...
            fireEvent = function (event) {
				// if (area.type_area == 'textArea'){
				// 	$image.trigger(event, [area.id, parent.areas()]);
				// } else if (area.type_area == 'imageArea'){
				// 	$image.trigger(event, [area.id, parent.areas()]);
				// } else if (area.type_area == 'qrcode'){
				// 	$image.trigger(event, [area.id, parent.areas()]);
				// } else {
				// 	$image.trigger(event, [area.id, parent.areas()]);
				// }
            ...
            updateSelection = function () {
                ...
                if (area.type_area == 'textArea' || area.type_area == 'shortcodeArea' ){
                    $textArea
                        .text(area.textArea.content)
                        .css({
                            'color': area.textArea.color,
                            'font-size': area.textArea.size,
                            'font-family' : area.textArea.font,
                            'font-style' : area.textArea.style,
                            'margin' : area.textArea.margin,
                            display: 'block'
                        });
                } else if (area.type_area == 'imageArea'){
                    $imageArea.attr('src', area.imgArea.src).css('display', 'block');
                } else if (area.type_area == 'qrCode'){
                    var sizeEncode = encodeURIComponent();
                    $qrcodeArea.attr('src', 'https://chart.googleapis.com/chart?chs='+ Math.round(area.width) +'x'+ Math.round(area.height) +'&cht=qr&chl=magenest').css('display', 'block');
                }
            },
            ...
            editSelection = function (event, id, options) {
                cancelEvent(event);
    ...
    $.imageSelectAreas.prototype._edit = function (id, options, silent) {
		this.set(id, options, silent);
		this._refresh();
	};
	$.imageSelectAreas.prototype.edit = function (id, options) {
		if (this._areas[id]) {
			// this._areas[id].editSelection(id, options);
            this._edit(id,options, undefined);
		}
	};
