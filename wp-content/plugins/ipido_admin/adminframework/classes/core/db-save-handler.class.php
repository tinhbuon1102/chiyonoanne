...
/**
 * Class CSFramework_DB_Save_Handler
 */
class CSFramework_DB_Save_Handler extends CSFramework_Abstract {
	/**
	 * _instance
	 *
	 * @var null
	...
	/**
	 * @return null|\CSFramework_DB_Save_Handler
	 */
	...
	public function _sanitize_field( $field, $value ) {
		...
		if ( false !== $type && has_filter( 'csf_sanitize_' . $type ) ) {
			$value = apply_filters( 'csf_sanitize_' . $type, $value, $field );
		}
	...
	public function _validate_field( $field, $value ) {
		if ( isset( $field['validate'] ) && has_filter( 'csf_validate_' . $field['validate'] ) ) {
			$validate = apply_filters( 'csf_validate_' . $field['validate'], $value, $field );
			if ( ! empty( $validate ) ) {
