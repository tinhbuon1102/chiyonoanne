...
abstract class CSFramework_Abstract {
	...
	/**
	 * CSFramework_Abstract constructor.
	 */
	...
	/**
	 * Checks if current ajax request is done by CSF
	 *
	 * @return bool
	 */
	public function is_csf_ajax() {
		if ( isset( $_POST ) && isset( $_POST['action'] ) && 'CSF-ajax' === $_POST['action'] ) {
			return true;
		}
		return false;
	}
	...
	protected function _action() {
		return $this->csf_action_filter( 'do_action', func_get_args() );
	}
	...
	protected function _filter() {
		return $this->csf_action_filter( 'apply_filters', func_get_args() );
	}
	 ...
	 * @return mixed
	 */
	protected function csf_action_filter( $type = 'apply_filters' ) {
		$_args   = func_get_args();
		$args    = $_args[1];
	...
	/**
	 * Returns Custom Prefix For Every Action & Filter Applied By CSF
	 *
	...
	protected function get_action_filter_slugs( $plugin_id = false ) {
		if ( false === $plugin_id ) {
			return 'csf_' . $this->type . '_';
		}
		return 'csf_' . $this->type . '_' . $this->plugin_id . '_';
	}
