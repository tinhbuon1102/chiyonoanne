...
class CSFramework_Shortcode_Manager extends CSFramework_Abstract{
  ...
  public function __construct( $options ) {
    ...
    if( ! empty( $this->options ) ) {
      ...
      $this->addAction( 'wp_ajax_csf-get-shortcode', 'shortcode_generator', 99 );
  ...
  public static function instance( $options = array() ){
    if ( is_null( self::$instance ) && CSF_ACTIVE_SHORTCODE ) {
  ...
  public function media_shortcode_button( $editor_id ) {
    ...
    if( ! in_array( $post_type, $this->exclude_post_types ) ) {
      echo '<a href="#" class="button button-primary csf-shortcode" data-editor-id="'. $editor_id .'">'. __( 'Add Shortcode', 'csf-framework' ) .'</a>';
    }
  ...
  ?>
    <div id="csf-shortcode-dialog" class="csf-dialog" title="<?php _e( 'Add Shortcode', 'csf-framework' ); ?>">
      <div class="csf-dialog-header">
        <select class="<?php echo ( is_rtl() ) ? 'chosen-rtl ' : ''; ?>csf-dialog-select" data-placeholder="<?php _e( 'Select a shortcode', 'csf-framework' ); ?>">
          <option value=""></option>
          <?php
      ...
      <div class="csf-dialog-load"></div>
      <div class="csf-insert-button hidden">
        <a href="#" class="button button-primary csf-dialog-insert"><?php _e( 'Insert Shortcode', 'csf-framework' ); ?></a>
      </div>
    </div>
  <?php
  }
  ...
  public function shortcode_generator() {
    ...
    if( isset( $shortcode['clone_fields'] ) ) {
      ...
      echo '<div class="csf-clone-button"><a id="shortcode-clone-button" class="button" href="#"><i class="fa fa-plus-circle"></i> '. $shortcode['clone_title'] .'</a></div>';
