...
/**
 * Class CSFramework_Quick_Edit
 */
class CSFramework_Quick_Edit extends CSFramework_Abstract {
	...
	public function __construct( $options = array() ) {
		$this->options = apply_filters( 'csf_quick_edit_options', $options );
	...
	public function load_style_script() {
		...
		if ( 'edit.php' === $pagenow && isset( $this->post_types[ $typenow ] ) ) {
			...
			wp_enqueue_script( 'csf-quick-edit' );
		}
	}
	...
	public function render_hidden_data( $column, $post_id ) {
		...
		echo '<div id="csf_quick_edit_' . $post_id . '" class="hidden">';
	...
	public function render_quick_edit( $column, $post_type ) {
		...
		if ( false === $this->is_nonce_rendered ) {
			wp_nonce_field( 'csf-quick-edit', 'csf-quick-edit-nonce' );
			$this->is_nonce_rendered = true;
		}

		$options = $this->formatted[ $post_type ][ $column ];
		echo '<fieldset class="inline-edit-col-left"> <div class="csf_quick_edit_fields inline-edit-col"> ';
	...
	private function render_fields( $option, $db_key = '' ) {
		...
		foreach ( $option['fields'] as $field ) {
			$html .= csf_add_element( $field, '', $db_key );
		}
		return $html;
	}
	...
	public function save_quick_edit( $post_id, $post ) {
		if ( wp_verify_nonce( csf_get_var( 'csf-quick-edit-nonce' ), 'csf-quick-edit' ) ) {
			$post_type = csf_get_var( 'post_type' );
			$validator = new CSFramework_DB_Save_Handler;
			if ( isset( $this->post_types[ $post_type ] ) && isset( $this->formatted[ $post_type ] ) ) {
				foreach ( $this->formatted[ $post_type ] as $data ) {
					foreach ( $data as $section ) {
						...
						$submitted_val = csf_get_var( $request_key, array() );
						...
						$request = apply_filters( 'csf_save_post', $request, $request_key, $post );
