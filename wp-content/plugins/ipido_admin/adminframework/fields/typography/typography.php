...
class CSFramework_Option_typography extends CSFramework_Options {
  ...
  public function output() {
    ...
    if( is_object( $google_json ) ) {
      ...
      do_action( 'csf_typography_family', $family_value, $this );

      echo '<optgroup label="'. __( 'Web Safe Fonts', 'csf-framework' ) .'">';
      ...
      echo '<optgroup label="'. __( 'Google Fonts', 'csf-framework' ) .'">';
      ...
      if( ! empty( $is_variant ) ) {
        ...
        echo '<label class="csf-typography-variant">';
        echo '<select name="'. $this->element_name( '[variant]' ) .'" class="'. $is_chosen . $chosen_rtl .'csf-typo-variant" data-atts="variant">';
