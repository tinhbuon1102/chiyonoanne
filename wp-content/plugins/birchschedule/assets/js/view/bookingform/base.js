(function($) {
    ...
    var ns = birchpress.namespace('birchschedule.view.bookingform', {
        __init__: function() {
            ...
            $('#birs_book_appointment').click(function(e) {
            	var bookingForm = $("form#birs_appointment_form");
        		e.preventDefault();
        		bookingForm.validationEngine({promptPosition: 'inline', addFailureCssClassToField: "inputError", bindMethod:"live"});
        		var validate = bookingForm.validationEngine('validate');
        		if (validate)
        		{
        			ns.bookAppointment();
        		}
            ...
            $.post(ajaxUrl, postData, function(data, status, xhr) {
                ...
                if (result.errors) {
                	...
                	if ($('booking_confirming').length)
                	{
                		...
                		// Go to confirm page
                		//location.href = gl_site_url + "reservation-confirm";
                		$('form#birs_appointment_form').submit();
                	}
                }
                else {
                	$('body').LoadingOverlay('hide');
