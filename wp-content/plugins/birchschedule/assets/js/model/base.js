(function($) {
    ...
    var ns = birchpress.namespace('birchschedule.model', {
        ...
        isDayAvaliableByNow: function(date) {
            ...
            var paddingDayConfig = birchschedule_view_bookingform['days_minimum_services_map'][serviceId]['birs_service_day_avail'];
            paddingDayConfig = paddingDayConfig ? parseInt(paddingDayConfig) : 0;
            
            var aDate = serverToday.split('-');
            var paddingToday = new Date(aDate[0], aDate[1] - 1, aDate[2]);
            paddingToday.setDate(paddingToday.getDate() + paddingDayConfig)
            
            today = paddingToday.getFullYear() + '-' + ('0' + paddingToday.getMonth()).slice(-2) + '-' + ('0' + paddingToday.getDate()).slice(-2);
            ...
            var days_off = birchschedule_view_bookingform['days_off'];
            if (days_off[staffId] == selectedDay)
            {
            	return false;
            }
