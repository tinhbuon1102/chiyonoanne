...
birch_ns( 'birchschedule.model', function( $ns ) {
		...
		$ns->get_days_minimum_services_map = function() use( $ns, $birchschedule ) {
			$services = $birchschedule->model->query(
					array( 'post_type' => 'birs_service' ),
					array(
						'meta_keys' => array( '_birs_service_day_avail' ),
						'base_keys' => array()
					)
					);
			$day_map = array();
			foreach ( $services as $service ) {
				$birs_service_day_avail = $service['_birs_service_day_avail'];
				$day_map[$service['ID']] = array(
					'birs_service_day_avail' => $birs_service_day_avail,
				);
			}
			return $day_map;
		};
		
		$ns->get_days_off = function() use( $ns, $birchschedule ) {
			$unavaiable_dates = get_option('booking_unavaiable_dates');
			$unavaiable_dates = $unavaiable_dates ? $unavaiable_dates : array();
			return $unavaiable_dates;
		};
		...
		$ns->get_staff_daysoff = function( $staff_id ) {
			...
			if (!empty($offduty_dates))
			{
				$daysoff = json_decode($daysoff);
				foreach ($offduty_dates as $offduty_date)
				{
					$offduty_date = date('m/d/Y', strtotime($offduty_date));
					if (!in_array($offduty_date, $daysoff))
					{
						$daysoff[] = $offduty_date;
					}
				}
