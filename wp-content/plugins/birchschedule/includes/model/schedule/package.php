...
birch_ns( 'birchschedule.model.schedule', function( $ns ) {
		...
		$ns->get_avaliable_time_options = function( $schedule, $service_id ) use ( $ns ) {
			...
			$current_time = date('H', strtotime(current_time('mysql'))) * 60 + date('i', strtotime(current_time('mysql'))) + date('s', strtotime(current_time('mysql'))) / 60;
			
			$booking_date = $_REQUEST['birs_appointment_date'];
			$booking_date = date('Y-m-d', strtotime($booking_date));
			if ($current_date == $booking_date)
			{
				$start = $start < $current_time ? $start + $timeslot : $start;
			}
