...
birch_ns('birchschedule.view.clients', function($ns) {
		...
		$ns->get_client_details_html = function($client_id) {
			...
			$client_email = get_post_meta($client_id, '_birs_client_email', true);
...
?>
				...
				{
					if ($field['parent_name'] == 'q_04' && $user)
					{
						?>
						<div class="form-row">
							<div class="field-wrapper">
							<?php
							$field['value'] = (array)get_user_meta($user->ID, $field['name'], true);
...
foreach ($appointment1on1s as $index => $appointment1on1)
{
	$appointment_id = get_post_meta($appointment1on1->ID, '_birs_appointment_id', true);
	$appointment = get_post($appointment_id);
	if ($appointment->post_status == 'publish')
	{
		$time = get_post_meta($appointment_id, '_birs_appointment_timestamp', true);
		$appointmentInfos[$index]['time'] = date('Y-m-d H:i', $time);
		$appointmentInfos[$index]['link'] = site_url('wp-admin/post.php?post='.$appointment_id.'&action=edit');
	}
}
$statuses = wc_get_order_statuses();
        ...
        <div class="order_wraper">
        	<h2>Orders (<?php echo count($customer_orders)?>)</h2>
        	<div class="order_list">
        		...
        		?>
        			<table>
        				<thead>
        					<th><?php _e('Order', 'birchschedule')?></th>
	        				<th><?php _e('Date', 'birchschedule')?></th>
	        				<th><?php _e('Status', 'birchschedule')?></th>
	        				<th><?php _e('Total', 'birchschedule')?></th>
        				</thead>
        				<tbody>
        					<td><a target="_blank" href="<?php echo site_url('wp-admin/post.php?post='.$customer_order->ID.'&action=edit')?>">#<?php echo $customer_order->ID?></a></td>
	        				<?php /* ?><td><?php echo human_time_diff(get_the_time('U', $customer_order), current_time('timestamp'));?></td><?php */?>
	        				<td><?php echo date('Y-m-d H:i', strtotime($customer_order->post_date))?></td>
	        				<td><?php echo $statuses[$customer_order->post_status]?></td>
        ...
        <div class="order_wraper">
        	<h2>Appointments (<?php echo count($appointmentInfos)?>)</h2>
        	<div class="order_list">
        		<?php foreach ($appointmentInfos as $appointmentInfo) {?>
        			<table>
        				<thead>
        					<th><?php _e('Time', 'birchschedule')?></th>
        				</thead>
        				<tbody>
        					<td><a target="_blank" href="<?php echo $appointmentInfo['link']?>"><?php echo $appointmentInfo['time']?></a></td>
        				</tbody>
        			</table>
        		<?php }?>
        	</div>
