...
birch_ns( 'birchschedule.view.services', function( $ns ) {
		...
		$ns->save_post = function( $post ) {
			...
			$config = array(
				...
				'meta_keys' => array(
					...
					'_birs_assigned_staff', '_birs_service_day_avail',
					'_birs_service_day_cancel_before', '_birs_service_time_cancel_before'
				)
			);
			$service_data =
		...
		$ns->render_service_info = function( $post ) use( $ns ) {
			...
			$day_avail = get_post_meta( $post_id, '_birs_service_day_avail', true );
			$day_cancel_before = get_post_meta( $post_id, '_birs_service_day_cancel_before', true );
			$time_cancel_before = get_post_meta( $post_id, '_birs_service_time_cancel_before', true );
?>
        <div class="panel-wrap birchschedule">
            <table class="form-table">
                ...
                <tr class="form-field">
                    <th><label><?php echo __( 'Time Range', 'zoa' ); ?> </label></th>
                    <td>
                        <input type="text"
                               name="birs_service_price" id="birs_service_price" value="<?php echo $price; ?>"> <?php echo __( 'Minutes', 'birchschedule' ); ?> 
                    </td>
                </tr>
                <tr class="form-field">
                    <th><label><?php echo __( '最短予約可能日', 'zoa' ); ?> </label></th>
                    <td>
                        <input type="text" style="width: 40%"
                               name="birs_service_day_avail" id="birs_service_day_avail" value="<?php echo $day_avail; ?>"> (日前)
                    </td>
                </tr>
                <tr class="form-field">
                    <th><label><?php echo __( 'Cancellable term', 'zoa' ); ?> </label></th>
                    <td>
                    	<?php echo __('Until', 'zoa')?>
                        <input style="width: 40%" type="text" name="birs_service_day_cancel_before" id="birs_service_day_cancel_before" value="<?php echo $day_cancel_before; ?>"> <?php echo __('days before and', 'zoa')?>
                        <select name="birs_service_time_cancel_before" id="birs_service_time_cancel_before">
                        	<?php for($i = 0; $i <= 23; $i ++) {?>
                        		<option value="<?php echo $i?>" <?php echo ($i == (int)$time_cancel_before) ? 'selected' : ''?>><?php echo $i?>:00</option>
                        	<?php }?>
                        </select> <?php echo __('hours', 'zoa')?>
                    </td>
                </tr>
            </table>
        </div>
