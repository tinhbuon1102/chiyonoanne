...
birch_ns( 'birchschedule.view.bookingform', function( $ns ) {
		...
		$ns->get_script_data_fn_view_bookingform = function() use ( $ns ) {
			...
			return array(
				...
				'days_minimum_services_map' => $ns->get_days_minimum_services_map(),
				'days_off' => $ns->get_days_off(),
		...
		$ns->get_days_minimum_services_map = function() {
			global $birchschedule;

			return $birchschedule->model->get_days_minimum_services_map();
		};
		
		$ns->get_days_off = function() {
			global $birchschedule;

			return $birchschedule->model->get_days_off();
		};
...
?>
		...
		$ns->schedule = function() use ( $ns ) {
			...
			$appointment_id = get_post_meta($appointment1on1_id, '_birs_appointment_id', true);
			do_action('after_save_booking', $appointment_id, $client_id, $appointment1on1_id);
...
?>
                <div class="birchschedule" id="birs_booking_box">
                    <form id="birs_appointment_form" action="<?php echo site_url()?>/reservation-confirm" method="post" class="form form--stepped validate">
...
<fieldset class="step" id="step-1">
	<div class="row rv_form_row">
		<div class="col-lg-6 col-md-5 col-xs-12 form_sideimg" style="background-image: url(<?php echo $step01['image']; ?>);">
			<div class="sideimg__content text-center">
				<?php
			the_title( '<h1 class="page-title entry-title">', '</h1>' );
		//the_content();
			echo '<p class="rv_item_body">' . $step01['text'] . '</p>';
	?>
				<a href="<?php echo home_url('/bespoke'); ?>" class="more_details double_button"><?php _e('See more details', 'zoa'); ?></a>
			</div>
		</div>
		<div class="col-lg-6 col-md-7 col-xs-12 form_sidecon">
	<legend class="hidden"><?php _e( 'Appointment Info', 'zoa' ); ?></legend>
                <ul>
                <li class="birs_form_field birs_appointment_section">
                    <h2 class="birs_section"><?php _e( 'Appointment Info', 'zoa' ); ?></h2>
                </li>
                <li class="birs_form_field birs_appointment_location">
                    <label><?php echo __($labels['location'], 'zoa'); ?></label>
                ...
                <li class="birs_form_field birs_appointment_service">
                    <label><?php echo __($labels['service'], 'zoa'); ?></label>
                    <div class="birs_field_content">
                        <div class="selectric-wrapper selectric-input-select selectric-responsive">
							
								<select id="birs_appointment_service" name="birs_appointment_service" class="input-select justselect"></select>
							
						</div>
                    </div>
                </li>
                <li class="birs_form_field birs_appointment_staff">
                    <label><?php echo __($labels['service_provider'], 'zoa'); ?></label>
                    <div class="birs_field_content">
                ...
                <li class="birs_form_field birs_appointment_date">
                    <label><?php echo __($labels['date'], 'zoa'); ?></label>
                ...
                <li class="birs_form_field birs_appointment_policy">
                    <label><?php echo __('Terms of Service', 'zoa'); ?></label>
                    <div class="cancel_policy">
						<?php 
						$options = get_option( 'birchschedule_options' );
						echo nl2br(@$options['cancel_policy']);?>
					</div>
                </li>
                <li class="birs_form_field birs_appointment_policy_agree">
                    <label class="check-label cancel_policy_agree">
						<input type="checkbox" value="1" id="cancel_policy_agree_checkbox"/>
						<?php echo __('Agree to the Terms of Service', 'zoa'); ?>
					</label>
                </li>
                
				</ul>
			
			<div class="btn-group birs_field_content">
					<input type="button" class="btn btn--inverse btn--1 btn--prev js-prev" value="<?php _e( 'Previous', 'zoa' ); ?>" />
					<input type="button" class="btn btn--next js-next disabled" value="<?php _e( 'Next', 'zoa' ); ?>" />
                </div>
		</div><!--/.form_sidecon-->
	</div><!--/row-->
</fieldset>
<fieldset class="step" id="step-2">
	<div class="row rv_form_row">
	<div class="col-lg-6 col-md-5 col-xs-12 form_sideimg" style="background-image: url(<?php echo $step02['image']; ?>);">
			<div class="sideimg__content text-left">
				<h1 class="page-title entry-title"><?php _e( 'Fill your information', 'zoa' ); ?></h1>
				<?php echo '<p class="rv_item_body">' . $step02['text'] . '</p>'; ?>
			</div>
		</div>
	<div class="col-lg-6 col-md-7 col-xs-12 form_sidecon">
	<legend class="hidden"><?php _e( 'Your Info', 'zoa' ); ?></legend>
				<ul>
                <li class="birs_form_field birs_client_section">
                    <h2 class="birs_section"><?php _e( 'Your Info', 'zoa' ); ?></h2>
                </li>
                <li>
                	<?php get_booking_form(2);
                	global $birchschedule;
				...
				<?php if($_SESSION['p_image']!=''){?>
					<li>
						<div class="acf-field">
							<div class="acf-label"><label><?php _e( 'Inspired Photo', 'zoa' ); ?></label></div>
							<div class="acf-input">
								<img src="<?php echo $_SESSION['p_image'];?>" alt="image">
								<span class="delete_photo">X</span>
							</div>
						</div>
					</li>	
				<?php } ?>
                ...
                <li class="birs_footer">
                    ...
                    <div style="display:none;" id="birs_please_wait"><?php _e( 'Please wait...', 'zoa' ); ?></div>
                    <?php 
		...
		<div class="btn-group birs_field_content">
					<input type="button" class="btn btn--inverse btn--1 btn--prev js-prev" value="<?php _e( 'Previous', 'zoa' ); ?>" />
					<input type="button" class="btn btn--next js-next" value="<?php _e( 'Next', 'zoa' ); ?>" />
                </div>
	</div><!--/.form_sidecon-->
	</div><!--/row-->
...
<fieldset class="step" id="step-3">
	<div class="row rv_form_row">
		<div class="col-xs-12 form_sidecon">
	<legend class="hidden"><?php _e( 'Your Inquiry', 'zoa' ); ?></legend>
	<ul>
		<li class="birs_form_field birs_client_section">
			<h2 class="birs_section"><?php _e( 'Your Inquiry', 'zoa' ); ?></h2>
		</li>
		<li><?php get_booking_form(3);?></li>
	</ul>
	<div class="btn-group birs_field_content">
		<input type="button" class="btn btn--inverse btn--1 btn--prev js-prev" value="<?php _e( 'Previous', 'zoa' ); ?>" />
		<input type="button" value="<?php _e( 'Confirm', 'zoa' ); ?>" class="btn btn_submit" id="birs_book_appointment">
     </div>
	</div><!--/.form_sidecon-->
	</div><!--/row-->
</fieldset>
