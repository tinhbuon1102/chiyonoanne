...
class MarketPress_Auto_Update {
	...
	public function setup( $plugindata ) {
		...
		// Setting up the license checkup URL
		$this->url_key_check = 'https://marketpress.com/mp-key/' . $parameter_string;
		$this->url_update_check = 'https://marketpress.com/mp-version/' . $parameter_string;
		$this->url_update_package = 'https://marketpress.com/mp-download/' . $this->key . '/' . $this->sanitized_plugin_name . '/' . sanitize_title_with_dashes( network_site_url() ). '/' . $plugindata->version. '/' . $phpversion;
	...
	public function license_key_checkup( $key = '' ) {
		...
		$this->url_key_check = 'https://marketpress.com/mp-key/' . $this->key . '/' . $this->sanitized_plugin_name . '/' . sanitize_title_with_dashes( network_site_url() );
		...
		if ( isset( $_GET[ 'message_mp_' .$this->plugin_data->shortcode ] ) ) {
			...
			switch ( $_GET[ 'message_mp_' .$this->plugin_data->shortcode ] ) {
				...
				case 'marketpress_wrong_url':
					...
					$class = 'error';
					$msg   = sprintf( '<strong>%s</strong> ', __( 'License cannot be activated.', 'marketpress-autoupdater' ) );
					$msg  .= sprintf( __( 'You have reached the limit of URLs included in your license. Please update your license at %s.', 'marketpress-autoupdater' ),
						'<a href="https://' . $mp_url . '/" target="_blank">MarketPress</a>'
					);
					break;
