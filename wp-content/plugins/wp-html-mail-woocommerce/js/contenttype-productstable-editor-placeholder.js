jQuery(document).ready(function($) {
    tinymce.PluginManager.add('mb_productstable_placeholder' , function( editor, url ) {
        if ( editor.id == 'mb_tiny_wysiwyg_editor' ){

            // add events
            var editor_menu = haet_mb_data.placeholder_menu.productstable;
            for(var i = 0; i<editor_menu.length; i++){
                if ( editor_menu[i].tooltip ){
                    editor_menu[i].onclick = function() {
                        ...
                        tinyMCE.get('mb_tiny_wysiwyg_editor').insertContent( this._aria.label );
                    }
                }
                if ( editor_menu[i].menu ){
                    for(var ii = 0; ii<editor_menu[i].menu.length; ii++){
                        ...
                        if ( editor_menu[i].menu[ii].menu ){
                            for(var iii = 0; iii < editor_menu[i].menu[ii].menu.length; ii++){
            ...
            editor.addButton( 'mb_productstable_placeholder', {
