...
if ( ! class_exists( 'YITH_WAPO_Frontend' ) ) {
	...
	class YITH_WAPO_Frontend {
		...
		public function __construct( $version ) {
			...
			add_filter( 'woocommerce_add_cart_item_data', array( $this, 'add_cart_item_data' ), 100000000, 2 );
		...
		public function printOptions(	$key,
			...
			if ( ! empty( $price ) || apply_filters( 'yith_wapo_allow_frontend_free_price', false ) ) {
				...
				if ( get_option('woocommerce_price_display_suffix') ) {
					...
					$price_including_tax = wc_get_price_including_tax( $product, array( 'price' => $simple_price ) );
					$price_excluding_tax = wc_get_price_excluding_tax( $product, array( 'price' => $simple_price ) );
