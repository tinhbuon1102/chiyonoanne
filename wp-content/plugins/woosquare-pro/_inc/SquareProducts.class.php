...
class WooSquare_API_Exception extends Exception {
...
class WooSquare_WC_Products {
	...
	public function create_product( $data ) {
		...
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product' ), 400 );
			}
			...
			if ( ! current_user_can( 'publish_products' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_create_product', __( 'You do not have permission to create products', 'woocommerce' ), 401 );
			}
			...
			if ( ! isset( $data['title'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_title', sprintf( __( 'Missing parameter %s', 'woocommerce' ), 'title' ), 400 );
			}
			...
			if ( ! in_array( wc_clean( $data['type'] ), array_keys( wc_get_product_types() ) ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_type', sprintf( __( 'Invalid product type - the product type must be any of these: %s', 'woocommerce' ), implode( ', ', array_keys( wc_get_product_types() ) ) ), 400 );
			}
			...
			if ( is_wp_error( $id ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_cannot_create_product', $id->get_error_message(), 400 );
			}
		...
		} catch ( WooSquare_API_Exception $e ) {
	...
	public function edit_product( $id, $data ) {
		try {
			if ( ! isset( $data['product'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_data', sprintf( __( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product' ), 400 );
			}
	...
	public function get_product_categories( $fields = null ) {
		try {
			...
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_read_product_categories', __( 'You do not have permission to read product categories', 'woocommerce' ), 401 );
			}
	...
	public function get_product_category( $id, $fields = null ) {
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_category_id', __( 'Invalid product category ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $term ) || is_null( $term ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_category_id', __( 'A product category with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	public function create_product_category( $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_category_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_category' ), 400 );
			}

			// Check permissions
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_create_product_category', __( 'You do not have permission to create product categories', 'woocommerce' ), 401 );
			}
			...
			if ( $data['parent'] ) {
				...
				if ( ! $parent ) {
					throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_category_parent', __( 'Product category parent is invalid', 'woocommerce' ), 400 );
				}
			}
			...
			if ( is_wp_error( $insert ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_cannot_create_product_category', $insert->get_error_message(), 400 );
			}
	...
	public function edit_product_category( $id, $data ) {
		...
		try {
			if ( ! isset( $data['product_category'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_category', sprintf( __( 'No %1$s data specified to edit %1$s', 'woocommerce' ), 'product_category' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_edit_product_category', __( 'You do not have permission to edit product categories', 'woocommerce' ), 401 );
			}
			...
			if ( is_wp_error( $update ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_cannot_edit_product_catgory', __( 'Could not edit the category', 'woocommerce' ), 400 );
			}
	 ...
	 * @param  array $data
	 * @return bool
	 * @throws WooSquare_API_Exception
	 */
		...
		if ( isset( $data['sku'] ) ) {
			...
			if ( '' == $new_sku ) {
				...
				if ( ! empty( $new_sku ) ) {
					...
					if ( ! $unique_sku ) {
						throw new WooSquare_API_Exception( 'woocommerce_api_product_sku_already_exists', __( 'The SKU already exists on another product', 'woocommerce' ), 400 );
					} else {
	...
	protected function save_variations( $id, $data ) {
		...
		foreach ( $variations as $menu_order => $variation ) {
			...
			if ( is_wp_error( $variation_id ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_cannot_save_product_variation', $variation_id->get_error_message(), 400 );
			}
					...
					if ( isset( $image['position'] ) && $image['position'] == 0 ) {
						if ( isset( $image['src'] ) ) {
							...
							if ( is_wp_error( $upload ) ) {
								throw new WooSquare_API_Exception( 'woocommerce_api_cannot_upload_product_image', $upload->get_error_message(), 400 );
							}
	...
	protected function upload_image_from_url( $image_url, $upload_for = 'product_image' ) {
		...
		if ( ! $parsed_url || ! is_array( $parsed_url ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_' . $upload_for, sprintf( __( 'Invalid URL %s', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( is_wp_error( $response ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_remote_' . $upload_for, sprintf( __( 'Error getting remote image %s.', 'woocommerce' ), $image_url ) . ' ' . sprintf( __( 'Error: %s.', 'woocommerce' ), $response->get_error_message() ), 400 );
		} elseif ( 200 !== wp_remote_retrieve_response_code( $response ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_remote_' . $upload_for, sprintf( __( 'Error getting remote image %s.', 'woocommerce' ), $image_url ), 400 );
		}
		...
		if ( ! $wp_filetype['type'] ) {
			...
			if ( ! $wp_filetype['type'] ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_' . $upload_for, __( 'Invalid image type.', 'woocommerce' ), 400 );
			}
		}
		...
		if ( $upload['error'] ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_' . $upload_for . '_upload_error', $upload['error'], 400 );
		}
		...
		if ( 0 == $filesize ) {
			@unlink( $upload['file'] );
			unset( $upload );
			throw new WooSquare_API_Exception( 'woocommerce_api_' . $upload_for . '_upload_file_error', __( 'Zero size file downloaded', 'woocommerce' ), 400 );
		}
	...
	public function get_product_attributes( $fields = null ) {
		try {
			// Permissions check.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_read_product_attributes', __( 'You do not have permission to read product attributes', 'woocommerce' ), 401 );
			}
	...
	public function get_product_attribute( $id, $fields = null ) {
		...
		try {
			...
			// Validate ID
			if ( empty( $id ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'Invalid product attribute ID', 'woocommerce' ), 400 );
			}
			...
			if ( is_wp_error( $attribute ) || is_null( $attribute ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_id', __( 'A product attribute with the provided ID could not be found', 'woocommerce' ), 404 );
			}
	...
	protected function validate_attribute_data( $name, $slug, $type, $order_by, $new_data = true ) {
		if ( empty( $name ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_attribute_name', sprintf( __( 'Missing parameter %s', 'woocommerce' ), 'name' ), 400 );
		}

		if ( strlen( $slug ) >= 28 ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_too_long', sprintf( __( 'Slug "%s" is too long (28 characters max). Shorten it, please.', 'woocommerce' ), $slug ), 400 );
		} else if ( wc_check_if_attribute_name_is_reserved( $slug ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_reserved_name', sprintf( __( 'Slug "%s" is not allowed because it is a reserved term. Change it, please.', 'woocommerce' ), $slug ), 400 );
		} else if ( $new_data && taxonomy_exists( wc_attribute_taxonomy_name( $slug ) ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_slug_already_exists', sprintf( __( 'Slug "%s" is already in use. Change it, please.', 'woocommerce' ), $slug ), 400 );
		}

		// Validate the attribute type
		if ( ! in_array( wc_clean( $type ), array_keys( wc_get_attribute_types() ) ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_type', sprintf( __( 'Invalid product attribute type - the product attribute type must be any of these: %s', 'woocommerce' ), implode( ', ', array_keys( wc_get_attribute_types() ) ) ), 400 );
		}

		// Validate the attribute order by
		if ( ! in_array( wc_clean( $order_by ), array( 'menu_order', 'name', 'name_num', 'id' ) ) ) {
			throw new WooSquare_API_Exception( 'woocommerce_api_invalid_product_attribute_order_by', sprintf( __( 'Invalid product attribute order_by type - the product attribute order_by type must be any of these: %s', 'woocommerce' ), implode( ', ', array( 'menu_order', 'name', 'name_num', 'id' ) ) ), 400 );
		}
	...
	public function create_product_attribute( $data ) {
		...
		try {
			if ( ! isset( $data['product_attribute'] ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_missing_product_attribute_data', sprintf( __( 'No %1$s data specified to create %1$s', 'woocommerce' ), 'product_attribute' ), 400 );
			}
			...
			// Check permissions.
			if ( ! current_user_can( 'manage_product_terms' ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_user_cannot_create_product_attribute', __( 'You do not have permission to create product attributes', 'woocommerce' ), 401 );
			}
			...
			// Checks for an error in the product creation.
			if ( is_wp_error( $insert ) ) {
				throw new WooSquare_API_Exception( 'woocommerce_api_cannot_create_product_attribute', $insert->get_error_message(), 400 );
			}
