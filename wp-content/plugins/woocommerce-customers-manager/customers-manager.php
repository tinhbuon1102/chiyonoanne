...
if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', get_option('active_plugins'))) ||
    ...
    $wccm_id = 10965432;
    $wccm_name = "WooCommerce Customers Manager";
    $wccm_activator_slug = "wccm-activator";

    include_once( "classes/com/WCCM_Acf.php");
    include_once( "classes/com/WCCM_Global.php");
    require_once('classes/admin/WCCM_ActivationPage.php');

    add_action('init', 'wccm_wp_init');
    add_action('admin_notices', 'wccm_admin_notices');
    add_action('admin_menu', 'wccm_init_act');
    if (defined('DOING_AJAX') && DOING_AJAX)
        wccm_init_act();
}
//For appointments
require_once('classes/com/BOOKED_appointments.php');
//End
function wccm_admin_notices() {
    global $wccm_notice, $wccm_name, $wccm_activator_slug;
    if ($wccm_notice && (!isset($_GET['page']) || $_GET['page'] != $wccm_activator_slug)) {
        ?>
        <div class="notice notice-success">
            <p><?php echo sprintf(__('To complete the <span style="color:#96588a; font-weight:bold;">%s</span> plugin activation, you must verify your purchase license. Click <a href="%s">here</a> to verify it.', 'woocommerce-customers-manager'), $wccm_name, get_admin_url() . "admin.php?page=" . $wccm_activator_slug); ?></p>
        </div>
        <?php
    }
}
...
function wccm_init_act() {
    global $wccm_activator_slug, $wccm_name, $wccm_id;
    new WCCM_ActivationPage($wccm_activator_slug, $wccm_name, 'woocommerce-customers-manager', $wccm_id, WCCM_PLUGIN_PATH);
}
...
function wccm_setup() {
    ...
    global $wccm_page, $wccm_guest_to_registered_helper, $wccm_order_model, $wccm_customer_model, $wccm_product_model,
    $wccm_wpml_helper, $wccm_configuration_model, $wccm_checkout_page_addon, $wccm_products_list_page_addon,
    $wccm_product_page_addon, $wccm_orders_page_addon, $wccm_html_model, $wccm_order_details_addon, $wccm_cart_model;
...
function wccm_adjust_admin_css_header() {
    ...
    echo '.wp-list-table .column-login { width: 15%; }';
...
function wccm_wpuef_plugin_installed() {
    return function_exists('wccm_set_field');
}
