...
class WCCM_CustomerTable extends WP_List_Table {
    ...
    function column_login($item) {
        ...
        $locale = get_user_locale($item['ID']);
        ...
        return @sprintf('<a href="?page=' . $_REQUEST['page'] . '&customer=' . $item['ID'] . '&action=customer_details">' .
                        get_avatar($item['ID'], 64, "", false, array('class' => 'wccm_avatar_img')) . '
								<div class="wccm_customer_data_container">
									<span class="wccm_name_surname_container">' . $fullname . '</span>
    ...
    function get_columns() {
        $columns = array(
            ...
            'appointments_count' => __('#Appointments', 'woocommerce-customers-manager'),
    ...
    function months_and_roles_dropdown($post_type = 'shop_order') {
        ...
        if (in_array("administrator", $user_current->roles)) {
            ?>
        ...
        <div id="wccm_date_filter_box">
            ...
            <select name="m" id="filter-by-date">
                ...
                foreach ($months as $arc_row) {
                    ...
                    printf("<option %s value='%s' >%s</option>\n", selected($selected_date, $year . $month, false), $value_for_current_option,
                            /* translators: 1: month name, 2: 4-digit year */ sprintf('%1$s %2$d', $wp_locale->get_month($month), $year)
                    );
                }
                ?>
            </select>
    ...
    function prepare_items() {
        ...
        foreach ($customers as $customer) {
            ...
            $appoints_num = getTotalForCustomer($customer->ID);
    ...
    function render_page() {
        ...
        <?php if (/* !$this->data_filter_or_product_filter_enabled &&  !$this->customers_ids_to_use_as_filter */true): ?>
            ...
            <h2 class="nav-tab-wrapper">
                ...
                if (isset($this->filter_by_product_id))
                    echo '&filter-by-product=' . $this->filter_by_product_id;
                if (isset($_REQUEST['wccm_customers_ids']))
                    echo '&wccm_customers_ids=' . $_REQUEST['wccm_customers_ids'];
                if (isset($this->customers_emails))
                    echo '&wccm_customers_emails=' . $this->customers_emails;
                if (isset($this->ordes_starting_date))
                    echo '&wccm_start_date=' . $this->ordes_starting_date;
                if (isset($this->ordes_ending_date))
                    echo '&wccm_end_date=' . $this->ordes_ending_date;
                ?>'>Registered</a>
                <a class='nav-tab' href='?page=woocommerce-customers-manager&action=wccm-guests-list<?php
                if (isset($this->filter_by_product_id))
                    echo '&filter-by-product=' . $this->filter_by_product_id;
                if (isset($_REQUEST['wccm_customers_ids']))
                    echo '&wccm_customers_ids=' . $_REQUEST['wccm_customers_ids'];
