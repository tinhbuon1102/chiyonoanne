...
$image_2 = WOOF_EXT::$includes['taxonomy_type_objects']['image'];
//********************
?>
...
//excluding hidden terms
    ...
    <?php if (!empty($terms)): ?>
        <?php foreach ($terms as $term) : $inique_id = uniqid(); ?>
            ...
            $image = $image_2->get_term_image($term['term_id'], $term['taxonomy']);
            ...
            //***
            $styles = $image_2->get_term_style($term['term_id'], $term['taxonomy']);
            if (!empty($styles))
            {
                $styles = trim($styles);
            }
